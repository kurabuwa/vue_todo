    ...
    <div class="todo__inner">
      ...
      <main class="main">
        ...
        <div class="todos">
          ...
          <template v-else>
            <p class="todos__empty">やることリストは登録されていません</p>
          </template>
        </div>
      </main>
...
/*
・index.jsについて
    vueファイルを読み込み、vueインスタンスを生成する役割を持つ。elはマウントするDOMを指定する。
・一覧の作成
    vueインスタンスが作成された後に実行されるcreatedにgetで通信をして帰ってきた値をtodosに格納。其れを表示するためにv-ifを用いて表示するように変更。{data}の部分はアロー関数といい、ajaxでの通信をした際のオブエクトの中のdataを直接受け取っている。また、この書き方はES2015以降の書き方である。
    Promiseは非同期で処理の終了を待たずに次の処理ができてこの処理が終わったら結果を返すという仕組み。
・追加
    v-modelでフォーム部分をダブルバインドし、postでフォームの内容を登録し、フォームを初期化する
    Object.assignはオブジェクトをコピーするメソッドである。これによdataの書き換えが可能になる。
*/
</script>
